# GRID BREAKER — PROTOTYPE 1

> The original corridor-smasher prototype. The name GRID BREAKER now belongs
> to the rail-runner at `games/gridbreaker/`; this game remains playable here.
...
`shared/scores.js` tracks, in `localStorage`: **all-time best**, **best per
...
1. **Backend leaderboards** — replace `shared/scores.js` with an API client
